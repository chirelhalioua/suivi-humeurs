<template>
  <div>
    ...
    <div v-if="view === 'daily'" class="select-day-container">
      ...
      <select id="daySelect" v-model="selectedDay" @change="fetchMoodData" class="day-select">
    ...
    <div v-if="view === 'daily'" class="mood-display">
      <div class="mood-container">
        ...
        <div v-if="missingMorningMood" class="error-message">Humeur du matin manquante</div>
        <div v-if="missingEveningMood" class="error-message">Humeur du soir manquante</div>
      </div>
    </div>
    ...
    <div v-if="view === 'weekly'" class="weekly-view">
      <div class="weekly-mood">
        <div v-for="(day, index) in days" :key="index" class="weekly-day">
...
const getUserDataFromToken = async () => {
  ...
  try {
    const response = await axios.get('https://suivi-humeurs.onrender.com/api/auth/me', {
      headers: {
...
const getMoodById = async (humeurId) => {
  try {
    const response = await axios.get(`https://suivi-humeurs.onrender.com/api/humeurs/${humeurId}`);
    return response.data;
  } catch (error) {
...
// Fonction pour vérifier si un jour est dans le passé ou aujourd'hui
...
// Fonction pour récupérer les humeurs de l'utilisateur
const fetchMoodData = async () => {
  const user = await getUserDataFromToken(); // Récupérer les infos utilisateur depuis le token
  ...
  try {
    const response = await axios.get(`https://suivi-humeurs.onrender.com/api/humeurs_utilisateurs/${user._id}`, {
      headers: {
    ...
    // Récupérer les titres des humeurs pour chaque entrée
    for (const entry of response.data) {
      const dayIndex = new Date(entry.date).getDay(); // Utiliser l'index du jour directement
      const moodData = await getMoodById(entry.humeurId); // Récupérer les informations de l'humeur
    ...
    // Vérification des humeurs manquantes
...
// Réagir aux changements de jour
watch(selectedDay, () => {
  fetchMoodData();
});

// Changer de vue (journalier ou hebdomadaire)
